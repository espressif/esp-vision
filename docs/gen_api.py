...
def _render_module_toctrees() -> str:
    """Render API navigation from the firmware module target conditions."""
    out = [
    ...
    for module in MODULES:
        groups.setdefault(module_targets(module), []).append(module)

    for targets, modules in groups.items():
        if not targets:
            continue
        if targets == TARGETS:
            indent = ''
