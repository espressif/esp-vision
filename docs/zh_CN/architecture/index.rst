...
- **板级后端**\ （\ ``boards/<BOARD>/``\ ）：各板配置及真实的相机/显示/SD 卡实现。
  P4X 与 S31 使用 ``esp_video``/V4L2，P4X 还使用 PPA；S3 使用 ``esp32-camera``。
...
.. list-table::
   ...
   * - ``modules/``
     - MicroPython C/C++ 绑定（``sensor``、``image``、``display``、``imageio``、
       ``espdl``，以及随 target 启用的 ``h264`` 和 ``rtsp``）。
   * - ``components/``
...
``micropython.cmake`` 根据 ``IDF_TARGET`` 和板级配置选择模块。ESP32-P4 构建包含
``h264`` 与 ``rtsp``；当前 P4 板级配置还会启用 ZXing-C++ 条形码后端。最终公开 API
矩阵见 :doc:`../target-support/index`\ 。
