...
``mpconfigboard.h``。部分条件还取决于 ESP-IDF 版本或 SoC 能力宏。

ESP-IDF 5.5 提供更完整的标准 MicroPython 功能。除 ``network`` 模块、WLAN 和
socket 外，还启用 SSL/TLS、``cryptolib``、WebSocket、WebREPL 和 socket events。
``machine.I2S`` 与 ESP32 PCNT 也会在所选 SoC 支持时启用；支持的芯片还可使用
ESP-IDF 5.4 及以上版本提供的 I2C target 模式。

使用 ESP-VISION overlay 时，ESP-IDF 6.0 及以上版本仍支持标准 ``network`` 模块、
WLAN 和 socket；该 IDF 版本上的 SSL/TLS、``cryptolib``、WebSocket、WebREPL、
socket events、``machine.I2S`` 和 ESP32 PCNT 仍保持关闭。S31 port 还提供默认
hostname ``mpy-esp32s31``。

当前开发板头文件也都关闭了 Bluetooth 和 ESP-NOW；S31 开发板还关闭了
``machine.ADC`` 和 ``machine.ADCBlock``。开发板仍需具备相应网络硬件和固件配置，
``network.WLAN`` 才能建立连接。
