...
顶层 Makefile 在调用 `idf.py` 前执行 `prepare-micropython`：检查 `lib/micropython` 是否处于 MicroPython v1.28.0 commit `e0e9fbb17ed6fd06bb76e266ae554784c9c80804`，随后在 `build/micropython/` 下导出干净的 MicroPython 构建副本，并将 `overlay/micropython/` 应用到该副本。
